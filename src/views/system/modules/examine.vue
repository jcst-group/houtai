<template>
    ...
    >
        ...
        <div class='fater1' v-if="record.subTime">
            <div class="title">
                <span>实际执行时间：</span>
            </div>
        ...
        <div slot='footer'>
            ...
            <a-popconfirm :title="'确定' + cancelText + '?'" @confirm="handelCancel" v-if="cancelText && !lock">
                <a-button style="background:#d9534f;color:#fff;border-color:#d9534f">{{cancelText}}</a-button>
            </a-popconfirm>
            <a-popconfirm v-if="!lock" :title="'确定' + okText + '?'" @confirm="handleOk">
                <a-button style="background:#1ABC9C;color:#fff;border-color:#1ABC9C">{{okText}}</a-button>
            </a-popconfirm>
        </div>
    </a-modal>
</template>
...
<style scoped>
#container1 >>> .amap-info-content {
background-color:rgba(35, 35, 35, 0.6);
 border-radius:10px 10px;
box-shadow: rgba(255, 255, 255, 0.5) 0px 0px 10px;
}
#container1 >>> .amap-info-sharp{
