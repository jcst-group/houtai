  ...
  >
    <a-tabs v-model="activeKey">
      <a-tab-pane tab="采收信息" key="1">
              ...
              <a-form-item label='抽样标准'>
                <a-input v-decorator="[
                      'samplingCriteria',
                      {
                          ...
                          rules: [{ required: true, message: '抽样标准必须在30位内！',min:0,max:30}],
                      }
                  ]"/>
              </a-form-item>
            </div>
              ...
              <a-form-item label='检测图片'>
                <div class="upload-wrapper">
                  <a-upload
                    ...
                    :showUploadList="true"
                    action="/api/pictureInfo/addRecDetectionPic"
              ...
              <a-form-item label='检测结果'>
                <div class="upload-wrapper">
                  <a-upload
                    ...
                    class="avatar-uploader"
                    :showUploadList="true"
                    action="/api/pictureInfo/addRecDetectionResultPic"
                    :beforeUpload="beforeUpload"
                    :headers="headers"
                    :fileList='decFileList'
                    @change="handleDecChanges"
                    @preview='handleDecPreview'
                    :remove='remove'
                  >
                    <div v-if="decFileList.length < 1" style="transform: translateY(50%)">
                      <a-icon type="plus"/>
                      <div class="ant-upload-text">Upload</div>
                    </div>
                  </a-upload>
                  <a-modal :visible="imgDecVisible" :footer="null" @cancel="handleDecCancel">
                    <img alt="example" style="width: 100%" :src="imgDecUrl" />
                  </a-modal>
                </div>
              </a-form-item>
            </div>
        ...
        <div style="margin: 40px 0px 10px 16px;">
          <a-button
            type="primary"
            icon="plus"
            @click="addBatch"
    ...
    <RecoveryBatch ref="RecoveryBatch" @changeData="changeRecoveryDatas" @setData='getBatchDatas' :batchDatas='recoveryDatas'></RecoveryBatch>
  </a-modal>
</template>
<script>
  import {
  ...
  import RecoveryBatch from './addRecoveryBatch'
    ...
    methods: {
      open(record) {
        ...
        if (record) {
          ...
          this.curPlanId = record.planId
          getBatchByPlan({
            planId: record.planId
          }).then(res => {
            if (res.success) {
              this.recoveryDatas = res.result
              this.allRecoveryDatas = res.result
            }
          })
        } else {
          this.batchDatas = []
      ...
      addBatch(datas) {
        if (datas && !datas.type) {
          this.recoveryDatas = datas
        } else {
          this.recoveryDatas = this.allRecoveryDatas
        }
        this.$refs.RecoveryBatch.visible = true
        this.$refs.RecoveryBatch.rowSelection.selectedRowKeys = []
        this.$refs.RecoveryBatch.curPlanId = this.curPlanId
        this.recoveryDatas.forEach((item,index) => {
          for (let i = 0; i < this.batchDatas.length; i++) {
            if (item.id === this.batchDatas[i].id) {
              this.$refs.RecoveryBatch.rowSelection.selectedRowKeys.push(index)
            }
          }
        })
      },
      changeRecoveryDatas(datas) {
        this.addBatch(datas)
      },
      handleImgCancel () {
            ...
            if (!err) {
                if (this.title === '添加采收检测') {
                  ...
                  this.batchDatas.forEach(item => {
                    params.recDetRelList.push({
                      recoveryId: item.id
                    })
                  })
                  if (params.recDetRelList.length === 0) {
                    this.$message.info('必须选择要检测的批次')
                    return
                  }
                  addRecoveryDet(params).then(res => {
                    this.$message.info(res.message)
                    if (res.success) {
                      this.visible = false
                      this.curPlanId = null
                      this.recoveryDatas = []
                      this.allRecoveryDatas = []
                  ...
                  updateRecoveryDet(params).then(res => {
                    this.$message.info(res.message)
                    if (res.success) {
                      this.visible = false
                      this.curPlanId = null
