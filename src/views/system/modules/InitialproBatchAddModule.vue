              ...
              <a-form-item label='初加工批次名称'>
                <a-input placeholder="请输入初加工批次名称" v-decorator="[
                    'initialName',
                    {
                        ...
                        rules: [{
                        required: true, message: '初加工批次名称必须在30位内！',min:0,max:30
                        }],
                    }
                ]"/>
              </a-form-item>
              ...
              <div class="form-wrapper" style="width: 100%">
                <a-form-item label='备注'>
                  <a-textarea style="height: 80px;min-height: 80px;" :autosize="{ minRows: 4, maxRows: 4 }"
                    ...
                    'content',
                    {
                        rules: [{ required: true, message: '备注必须在200位内！',min:0,max:200 }],
        ...
        <div v-if="isSeedShow==false&&productHarvestDate!=''">
          ...
          >
            <template slot='yield' slot-scope="text,record">
              <div>
                <a-input-number class="yield02" v-model="record.aaa"/>
              </div>
            </template>
            <template slot='unit'>
              <div>
      ...
      changeHaves(id) {
        ...
        getCropProductByHarvestedId({harvestedId: id}).then(res => {
          if (res.success) {
            ...
            let productHarvestDate1 =[];
            productHarvestDate1 = res.result;
            for(let i=0;i<productHarvestDate1.length;i++){
              productHarvestDate1[i].aaa=0
            }
            this.productHarvestDate = productHarvestDate1;
            console.log(this.productHarvestDate)
            this.$nextTick(() => {
          ...
          this.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
              ...
              for (let i = 0; i < this.productHarvestDate.length; i++) {
                console.log(document.getElementsByClassName('yield02'))
