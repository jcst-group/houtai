                        ...
                        'crop.id',
                            {
                                ...
                                rules: [{ required: true, message: '请选择药材名称' }],
                            }
                        ]">
                        <a-select-option 
            ...
            <div class="form-wrapper">
                <a-form-item label='检测项目名称'>
                    ...
                    'item',
                    {
                        ...
                        rules: [{ required: true, message: '项目名称必须在30位内！',min:0,max:30}],
                    }
                ]" />
                </a-form-item>
            </div>
                ...
                <a-form-item label='检测单位'>
                    <a-auto-complete
                        ...
                        v-decorator="[
                            'unit',
                            {
                                initialValue:record.unit ? record.unit.displayName : null,
                                rules: [{ required: true, message: '请选择单位'}],
                            }
                        ]">
                        <template slot="dataSource">
                            <a-select-option v-for="item in unitDatas" :key="item.value" :value='item.displayName'>{{item.displayName}}</a-select-option>
                        </template>
                    </a-auto-complete>
                </a-form-item>
            </div>
    ...
    methods: {
        ...
        handleOk() {
            if (this.title === '添加检测标准') {
                ...
                params.unit = this.record.unit.displayName === params.unit ? this.record.unit.displayName : params.unit
                let par = params.unit
                params.unit = {}
                params.unit.displayName = par;
                delete params.crop
