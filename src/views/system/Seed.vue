  ...
  <div style="background:white;padding:10px 0 10px 0;" id="boxs">
      ...
      <div>
        ...
        <div class="select">
          <a-form
            :form="form"
            @submit="handleSearch">
            <div class="search-wrapper" style="width:300px; float:left;">
    ...
    <div>
      ...
      >
        ...
        <span slot="seedCode" slot-scope="text, record" style="text-align: center">
          <span>{{record.seedBatch.seedCode}}</span>
        </span>
        <span slot="baseInfo" slot-scope="text, record" style="text-align: center">
          <span>{{record.baseInfo ? record.baseInfo.baseName : '非自种'}}</span>
        </span>
        <span slot="batchCode" slot-scope="text, record" style="text-align: center">
          <span>{{record.cropBatch? record.cropBatch.batchName : '非自种'}}</span>
        </span>
        ...
        <span slot="detectionVoList03" slot-scope="detectionVoList" style="text-align: center" class="slot-box">
           <div class="slot-div"  v-for="(v,index) in detectionVoList" :key="index">
             <span :style="{color: v.actValue > v.maxValue ? '#d81e06' : (v.actValue < v.minValue ? '#2B9578' : 'black')}">{{v.actValue}}</span>
             <a-icon style="color:#d81e06" v-if="v.actValue > v.maxValue" type="arrow-up" />
             <a-icon style="color:#2B9578" v-if="v.actValue < v.minValue" type="arrow-down" />
           </div>
        </span>
      ...
      return {
        ...
        standardColumns: [
          ...
          },
          {
            ...
            dataIndex: 'seedCode',
            key: 'seedCode',
            scopedSlots: {customRender: 'seedCode'},
            align: "center",
          },
          {
            title: '基地名称',
