<template>
 <div>
    ...
    <a-select v-model="serpost" style="width:100%;">
      <a-select-option v-for="v in baseinfo" :value="v.id" :key="v.id" v-show="next">{{v.baseName}}</a-select-option>
       <a-select-option v-for="v in baseinfo" :value="v.bid" :key="v.bid" v-show="!next">{{v.bName}}</a-select-option>
    </a-select>
     </a-modal>
 </div>
</template>
