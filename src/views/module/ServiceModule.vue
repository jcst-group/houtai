<template>
  ...
  <template v-if="npw">
    ...
    <a-select v-model="serpost">
      <a-select-option v-for="v in servicepost" :key="v.id" :value="v.id" v-show="next">{{v.positionName}}</a-select-option>
      <a-select-option v-for="v in servicepost" :key="v.id" :value="v.pId" v-show="!next">{{v.pName}}</a-select-option>
    </a-select>
    </a-form-item>
    </template>
  </a-form>
