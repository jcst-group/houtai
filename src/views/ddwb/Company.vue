<template>
    <div>
        ...
        <a-table :pagination="false" :rowKey="record => record.id" :columns="columns" :dataSource="data" :loading="loading" bordered>
            ...
            <div slot="shop" slot-scope="text, record">
                <a :href="record.shopUrl" target="_blank" v-if="record.shopUrl">
                    <a-icon type="shop" style="margin-right:5px"/>商城连接
                </a>
                <span v-else>无</span>
            </div>
        ...
        <a-table @change="handleTableChange1" :pagination="ipagination1" :rowKey="record => record.id" :columns="sercolumns" :dataSource="serdata" :loading="serloading" bordered>
            <span slot="ation" slot-scope="text, record">
                ...
                <a-popconfirm title="是否删除该用肥情况？" @confirm="serdel(record.id)" okText="确认" cancelText="取消">
        ...
        >
         <div>公司简介图片：</div>
            <div style="height:100px;margin:20px 0 20px 0;">
                <a-upload
                    name="file"
                    action="/api/companyInfo/addComPicture"
        ...
        >
         <div>管理模式图片：</div>
                <div style="height:100px;margin:20px 0 20px 0;">
                    <a-upload
                        name="file"
                        action="/api/companyInfo/addMPicure"
                        listType="picture-card"
                        :fileList="mpfileList"
        ...
        >
                ...
                <div style="height:100px;margin:20px 0 20px 0;">
                    <a-upload
                        name="file"
                        action="/api/companyInfo/addHPicure"
                        listType="picture-card"
                        :fileList="hpfileList"
        ...
        >
                <div style="margin-bottom:20px;">公司介绍视频：</div>
                    <a-upload name="file" :fileList="cmp4list" :remove="removefile" :multiple="false" action="/api/companyInfo/uploadVideoCompany" :headers="headers" :data="cdata" :beforeUpload="cfilebeforeUpload" @change="filehandleChange">
        ...
        >
                     ...
                     <a-upload name="file" :fileList="pmp4list" :remove="removefile" :multiple="false" action="/api/companyInfo/uploadVideoProduct" :headers="headers" :data="cdata" :beforeUpload="pfilebeforeUpload" @change="filehandleChange1">
...
import { getCompanyList,delCompany,getFertilizersList,delFertilizers,delPic,delFile,getTeamList,delTeam } from '@/api/ddwbApi'
...
export default {
    ...
    methods:{
        ...
        pfilebeforeUpload (file) {
            ...
            const isLt50M = file.size / 1024 / 1024 < 50
            if (!isLt50M) {
                this.$message.error('视频大小不能大于10M')
            }
        ...
        loadData() {
            ...
            getCompanyList(param).then(res=>{
                ...
                if(this.data[0].videoCompany){
                    ...
                    let url='/api/fileInfo/getVideo?fileUrl='+mp4.fileUrl
                    let a={
                        uid: mp4.id,
                        name: mp4.fileName,
                        status: 'done',
                ...
                for(let i=0;i<this.data[0].honorPicture.length;i++){
                    ...
                    let url='/api/pictureInfo/show/'+pic.pictureUrl
                    let a={
                        uid: pic.id,
                        name: pic.pictureUrl,
                        status: 'done',
