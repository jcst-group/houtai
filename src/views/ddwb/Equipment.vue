<template>
 <div>
    <div class="addbut">
        ...
        <a-button class="btn" type="primary" @click="goEquipment">基站数据</a-button>
    </div>
    <div class="input">
...
<script>
  ...
  import {addEquipment,delEquipment,getEquipmentList, getBoardUrl}  from '@/api/ddwbApi'
  ...
  export default {
    ...
    methods: {
      ...
      goEquipment() {
        getBoardUrl().then(res => {
          console.log(res)
        })
        if (window.location.hostname.indexOf('192.168') !== -1) {
          window.open('http://' + window.location.hostname + ':8090/test')
          // location.href = 'http://' + window.location.hostname + ':8090/test'
        } else {
          getBoardUrl().then(res => {
            if (res.success) {
              window.open(res.result.constant)
            }
          })
        }
      },
