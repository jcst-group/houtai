<template>
  <div class="login_box" ref="loginBox">
    ...
    <div style="width: 100%;height: 200px;margin:auto;text-align: center;margin-top: 6%;position: absolute;top: 0%;">
      <img src="../assets/ddwb01.png" style="width: 70px;margin-top: -2%;margin-right: 2%">
      <span class="login_span">代代为本数据化管理中心</span>
...
import Footer from "@/components/layouts/LoginFooter"
  export default {
    beforeCreate () {
     ...
     mounted() {
       this.$refs.loginBox.style.backgroundSize = `${window.innerWidth}, ${window.innerHeight}`
     },
    data() {
      return {
        loginBtn: false,
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.$store.dispatch("user/login",values).then(()=>{
              this.$router.push({ path: 'ddwbhome' })
